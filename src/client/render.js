...
function drawRectangle(shape, { x, y, w, h }, ratio) {
  shape.graphics.beginStroke("#02eef4");
...
function drawBackground() {
  ...
  for(let i = 0; i < Constants.FIELD_LINES; i++) {
    ...
    if(i > 0) {
      ...
      drawCorner(corner, { mtx: end_x, mty: end_y, ltx: border.x, lty: border.y }, old_ratio);
      drawCorner(corner, { mtx: end_x+end_w, mty: end_y, ltx: border.x+start_w*ratio, lty: border.y }, old_ratio);
      drawCorner(corner, { mtx: end_x+end_w, mty: end_y+end_h, ltx: border.x+start_w*ratio, lty: border.y+start_h*ratio }, old_ratio);
      drawCorner(corner, { mtx: end_x, mty: end_y+end_h, ltx: border.x, lty: border.y+start_h*ratio }, old_ratio);
    }
...
function drawBall() {
  fillCommand = ball.graphics.beginRadialGradientFill(["#ab98fe","#5832fd"], [0, 1], -8, -8, 0, 0, 0, 35).command;
...
function renderBallMarker(b, me) {
  ...
  ballMarker.graphics.clear().beginStroke("#0275f4").setStrokeStyle(3*ratio)
...
export function playing() {
  ...
  stage.update();
  stage.on("stagemousemove", updateMouse);
  stage.on('stagemousedown', click);
  renderInterval = setInterval(render, 1000 / 60);
}

// Gets rid of all shapes and event listeners.
// Replaces game rendering with _____ rendering.
export function notPlaying(state = Constants.MSG_TYPES.MENU) {
  ...
  if(state === Constants.MSG_TYPES.WAITING) {
    ...
    renderInterval = setInterval(renderWaiting, 1000 / 60);
  }
